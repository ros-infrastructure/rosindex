...
layout: page
title: Tutorial
permalink: /help/tutorial/
breadcrumbs: ['help']
---

# ROS Index Tutorial

This guide introduces the standard way of searching for ROS package-level documentation from the home page of `index.ros.org`.
...
Locate the search bar "Search Packages" in the middle of the home page on [index.ros.org]({{ '/' | prepend: site.baseurl }}):

<br><br><img style="margin-left: 40px; margin-right:40px; margin-bottom: 15px;" src="{{ '/assets/ros_index_home.png' | prepend: site.baseurl }}" width="512px">

You can use this search bar to find specific packages or search for specific package features.

Package searches and lists are shown for specific ROS distributions. Click on the appropriate distribution name to select your desired ROS distribution.

For example if one wanted to do color processing on images they could either search `image_proc` or `image processing`.
After clicking the top package in the list, it will bring one to a page similar to [this]({{ '/p/image_proc/github-ros-perception-image_pipeline/#rolling' | prepend: site.base_url}}):

<br><br><img style="margin-left: 40px; margin-right:40px; margin-bottom: 15px;" src="{{ '/assets/package_info.png' | prepend: site.baseurl }}" width="512px">
